...
export function TodoItem({ todo, onToggle, onDelete, onEdit }: TodoItemProps) {
  ...
  return (
    <div className={`bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg transition-all duration-200 ${
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            ...
            <span className={`px-3 py-1 rounded-full text-sm font-medium ${categoryColors[todo.category]}`}>
              {todo.category.charAt(0).toUpperCase() + todo.category.slice(1)}
            </span>
            {todo.dueDate && (
              <span className="px-3 py-1 rounded-full text-sm font-medium bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 flex items-center gap-1">
